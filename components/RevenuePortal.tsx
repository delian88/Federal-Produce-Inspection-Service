...
import { UserRole, RevenueTransaction } from '../types';
...
const RevenuePortal: React.FC<RevenuePortalProps> = ({ role }) => {
  ...
  useEffect(() => {
    const fetchTxns = async () => {
      try {
        const data = await postgres.queryAll('transactions');
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[10px] md:text-sm text-slate-500">
            {isReviewer ? 'Reviewer Mode: Financial Oversight' : 'Administrative Portal: Production Environment'}
          </p>
        </div>
        {!isReviewer && (
      ...
      <div className="bg-white rounded-[24px] md:rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-20 text-center text-slate-400 font-bold animate-pulse">Querying Database...</div>
        ) : (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-slate-900 text-white p-6 md:p-8 rounded-[24px] md:rounded-3xl hover:shadow-lg transition-all">
          ...
          <p className="text-2xl md:text-4xl font-bold mt-2 md:mt-4">₦{((transactions.reduce((acc, t) => acc + (t.status === 'PAID' ? t.amount : 0), 0) * 0.2) / 1000000).toFixed(1)}M</p>
        ...
        <div className="bg-emerald-600 text-white p-6 md:p-8 rounded-[24px] md:rounded-3xl hover:shadow-lg transition-all">
          ...
          <p className="text-2xl md:text-4xl font-bold mt-2 md:mt-4">₦{((transactions.reduce((acc, t) => acc + (t.status === 'PAID' ? t.amount : 0), 0) * 0.8) / 1000000).toFixed(1)}M</p>
        ...
        <div className="bg-white border border-slate-200 p-6 md:p-8 rounded-[24px] md:rounded-3xl shadow-sm hidden lg:block">
          ...
          <p className="text-2xl md:text-4xl font-bold mt-2 md:mt-4 text-slate-900">
            {((transactions.filter(t => t.status === 'PAID').length / transactions.length) * 100).toFixed(0)}%
          </p>
